  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        desc="Detección, escalado y recomendaciones IA para anomalías de proceso, calidad y mantenimiento."
        actions={
          <div className="flex items-center gap-2">
            {critCount>0 && <Chip color="red"><span className="pulse-dot w-1.5 h-1.5 rounded-full bg-crit-400"/>{critCount} críticas</Chip>}
            {highCount>0 && <Chip color="amber">{highCount} altas</Chip>}
            {medCount>0  && <Chip color="cyan">{medCount} medias</Chip>}
            <button onClick={()=>actions.markAllAlertsRead().catch(()=>{})} className="panel rounded-md px-3 py-2 text-xs text-slate-300 hairline">{I.check} Marcar todo</button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-12 gap-4">
        <Card title="Bandeja" subtitle={`${visible.length} alertas`} className="col-span-12 xl:col-span-5" accent="cyan"
          action={
            <div className="flex gap-1">
              {['ALL','CRITICAL','HIGH','MEDIUM','LOW'].map(f=>(
                <button key={f} onClick={()=>setFilter(f)} className={`px-2 py-1 text-[10px] rounded ${filter===f?'bg-cyan2-400/15 text-cyan2-400 border border-cyan2-400/30':'text-slate-400 hover:text-white'}`}>{f}</button>
              ))}
            </div>
          }>
          <div className="space-y-2 max-h-[640px] overflow-y-auto pr-1">
            {visible.map(a=>{
              ...
              return (
                <div key={a.id} onClick={()=>setSelected(a.id)} className={`panel rounded-md p-3 cursor-pointer transition ${isSel?'border border-cyan2-400/30':'hover:bg-white/[0.02]'}`} style={isSel?{boxShadow:`0 0 16px ${sc}25`}:{}}>
        ...
        {sel && (
          <Card title={`Alerta · ${sel.id}`} subtitle={sel.machine} className="col-span-12 xl:col-span-7" accent={sel.sev==='CRITICAL'?'red':(sel.sev==='HIGH'?'amber':'cyan')}
            glow={sel.sev==='CRITICAL'?'shadow-glowCrit':''}>
            <div className="flex items-start gap-4 pb-4 hairline-bottom">
              <div className="w-12 h-12 rounded grid place-items-center" style={{color: sel.sev==='CRITICAL'?'#EF4444':'#F59E0B', background:`${sel.sev==='CRITICAL'?'rgba(239,68,68,0.12)':'rgba(245,158,11,0.12)'}`, boxShadow:`inset 0 0 0 1px ${sel.sev==='CRITICAL'?'rgba(239,68,68,0.4)':'rgba(245,158,11,0.4)'}`}}>{I.alert}</div>
              ...
              <div className="panel rounded p-3">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={x.l}>
                      <div className="flex justify-between text-[11px]"><span className="text-slate-300">{x.l}</span><span className="num" style={{color:x.col}}>r={x.c.toFixed(2)}</span></div>
            ...
            <div className="mt-4 panel-strong rounded p-4 border" style={{borderColor:'rgba(168,85,247,0.35)', boxShadow:'0 0 24px rgba(168,85,247,0.15)'}}>
              ...
              <div className="flex gap-2 mt-3">
                <button className="px-3 py-1.5 text-xs rounded text-[#06080F] font-semibold" style={{background:'linear-gradient(90deg,#A855F7,#22D3EE)', boxShadow:'0 0 12px rgba(168,85,247,0.5)'}}>Aplicar automático</button>
            ...
            <div className="mt-5">
              <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Timeline del evento</div>
              <div className="relative pl-4">
                <div className="absolute left-1 top-0 bottom-0 w-px bg-slate-700"/>
                {[
                  {t:'14:02:11', l:'Disparo automático · regla R1', c:'#EF4444'},
                  {t:'14:02:23', l:'Notificación enviada a Plant Engineer', c:'#22D3EE'},
                  {t:'14:03:01', l:'IA generó recomendación', c:'#A855F7'},
                  {t:'14:03:48', l:'Sistema sugirió orden de trabajo PM-2098', c:'#22D3EE'},
                ].map((e,i)=>(
                  <div key={i} className="relative pl-4 py-1.5">
                    <span className="absolute -left-[5px] top-2.5 w-2.5 h-2.5 rounded-full" style={{background:e.c, boxShadow:`0 0 6px ${e.c}`}}/>
                    <div className="flex items-center gap-2">
                      <span className="num text-[11px] text-slate-500">{e.t}</span>
                      <span className="text-xs text-slate-200">{e.l}</span>
                    </div>
                  </div>
                ))}
              </div>
